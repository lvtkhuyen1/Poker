...
let OnClickGameEmoticon = (game) => {
    ...
    if(game.bEnableMenu == true || game.bMobileRaiseButton == true || game.bEnableMenu == true)
    {
        return;
    }
    if (game.bEnableChat == false) {
...
let OnClickGamelog = (game) => {
    ...
    if(game.bEnableMenu == true || game.bMobileRaiseButton == true)
    {
        return;
    }
    if (game.bEnableChat == false) {
...
let OnClickShowCard = (game) => {
    ...
    if(game.bAbstentionWin == true && game.bRebuyTurn == false)
    {
...
let OnClickQuater = (game) => {
    ...
    {
        objectBetting = {strBetting:'Allin', iAmount:player.iGameCoin, timestamp: Date.now()};
    }
    else
    {
        objectBetting = {strBetting:'Quater', iAmount:iAmount, timestamp: Date.now()};
    }
    // game.SetRaiseButton();
...
let OnClickHalf = (game) => {
    ...
    {
        objectBetting = {strBetting:'Half', iAmount:iAmount, timestamp: Date.now()};
    }
    // game.SetRaiseButton();
    //game.CheckBettingCoin(iAmount);
    //
...
let OnClickFull = (game) => {
    ...
    {
        objectBetting = {strBetting:'Full', iAmount:iAmount, timestamp: Date.now()};
    }
...
let OnClickAllin = (game) => {
    ...
    let objectBetting = {strBetting:'Allin', iAmount:player.iGameCoin, timestamp: Date.now()};
...
let OnClickCall = (game) => {
    ...
    let objectBetting = {strBetting:'Call', iAmount:iAmount, timestamp: Date.now()};
...
let OnClickCheck = (game) => {
    ...
    let objectBetting = {strBetting:'Check', iAmount:0, timestamp: Date.now()};
...
let OnclickMobileRaise = (game) => {
    ...
    game.SetMobileRaiseButton();
    game.listBettingButtons[7].bEnable = false;
...
let OnClickPlus = (game) => {
    ...
    let minCoin = Math.ceil(game.iCallCoin * 2 / 10000) * 10000;
    ...
    } else if (currentAmount % 10000 != 0) {
        // 현재 금액이 10000원 단위가 아닌 경우, 올림
        currentAmount = Math.ceil(currentAmount / 10000) * 10000;
    } else {
        // 이미 10000원 단위인 경우, 10000원 추가
        currentAmount += 10000;
    }
...
let OnClickMinus = (game) => {
    ...
    if(game.iCallCoin > 0 && currentAmount <= minCoin)
    {
        ...
        // 현재 금액이 10000원 단위가 아닌 경우, 내림
        currentAmount = Math.floor(currentAmount / 10000) * 10000;
    } else {
        currentAmount -= 10000;
    }
...
let OnClickReserveBetTrue = (game) => {

    if(game.bEnableMenu == true || game.bMobileRaiseButton == true)
    {
        return;
    }
    if(game.kMainUser != null)
    {
        game.bReserveBet = false;
        game.socket.emit('CM_ReserveBet', game.bReserveBet);
        soundClick.play();
    }
}

let OnClickReserveBetFalse = (game) => {
