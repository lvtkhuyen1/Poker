class IRobot{
    ...
    isHoleCardInRange(holeCards, position) {
        ...
        console.log(`iRobot :: isHoleCardInRange`);
    ...
    async Update() {
        ...
        if (this.bConnected == false ){
            ...
            if(this.fElapsedTime > 3) {
                ...
                let availableRooms = this.listRooms.filter(room => room.iNumPlayers < room.iMaxPlayers && room.iNumPlayers < 8 && this.account.iLevel == room.iDefaultCoin);
                if (availableRooms.length > 0) {
    ...
    async RequestMakeGame()
    {
        ...
        console.log(`iRobot :: RequestMakeGame - iDefaultCoin : ${this.account.iLevel}`);

        let data = {
            //eGameType:0,
            eGameType:'HOLDEM',
            iDefaultCoin:this.account.iLevel, 
    ...
    OnIO()
    {
        ...
        this.socket.on('SM_RequestLogin', (data) => {
            ...
            this.socket.emit('CM_Login', this.account.strID, this.account.strNickname, this.account.strPassword, this.account.iAvatar, this.account.eUserType);
        });
        ...
        this.socket.on('SM_LeaveGame', (strID) => {

            //console.log('SM_LeaveGame');
            // this.socket.bConnection = false;
            //this.listAbnormalID.push(strID);
            // this.socket.emit('CM_LeaveGame', data.strID);
        });
        ...
        this.socket.on('SM_BroadcastLeaveUser', (objectPlayer) => {
            ...
            //console.log(objectPlayer);
            //console.log("removeRobot function:", window.removeRobot); // Check if it's accessible

            if(objectPlayer.strID != this.account.strID)
            {
                //this.socket.emit('CM_LeaveGame', objectPlayer.strID);
                window.removeRobot(objectPlayer);
            }
        });
        ...
        this.socket.on('SM_EnableBetting', (objectData) => {
            ...
            try {
                ...
                console.log(`strID : ${this.socket.strID}`);
                console.log(`this.account : ${this.account.strID}`);
                ...
                this.iCoin = objectData.iCoin;
                // this.fRandomTime =1 + Math.random()*6;
                this.fRandomTime =1 + Math.random()*1;

                // 'Raise'가 가능한지 확인
                const canRaise = objectData.listEnableBettingType.includes('Raise');
                ...
                if( objectData.iCoin > 0 )
                {
                    if (this.account.strID == objectData.strIDjoker) {
                        ...
                        if (this.isHoleCardInRange(holeCards, 'CO')) {
                            if (objectData.iCallAmount == 0) {
                                if (canRaise && randomType < 0.25) {
                                    bettingType = 'Raise';
                                    iCallAmount = parseInt((Math.floor(Math.random() * (59 - 21 + 1) + 21) * parseInt(objectData.iDefaultCoin)) / 10000) * 10000;
                                } else if (canRaise && randomType < 0.5) {
                                    bettingType = 'Half';
                                    iCallAmount = Math.floor(parseInt(objectData.iTotalBettingCoin / 2));
                                } else if(canRaise && randomType < 0.75){
                                    bettingType = 'Full';
                                    iCallAmount = Math.floor(parseInt(objectData.iTotalBettingCoin));
                                }
                                else
                                {
                                    this.fRandomTime = 1;
                                ...
                                // 랜덤하게 Raise, Call, Half, Full 중 하나 선택
                                if (canRaise && randomType < 0.25) {
                                    bettingType = 'Raise';
                                    iCallAmount = Math.ceil((parseInt(objectData.iCallAmount) + ((Math.floor(Math.random() * 3) + 1) * parseInt(objectData.iCallAmount))) / 10000) * 10000;
                                } else if (canRaise && randomType < 0.5) {
                                    bettingType = 'Full';
                                    iCallAmount = Math.floor(parseInt(objectData.iCallAmount)+(parseInt(objectData.iCallAmount) + parseInt(objectData.iTotalBettingCoin)));
                                } else if (canRaise && randomType < 0.75) {
                                    bettingType = 'Half';
                                    iCallAmount = Math.floor(parseInt(objectData.iCallAmount)+(parseInt(objectData.iCallAmount) + parseInt(objectData.iTotalBettingCoin / 2)));
                                } else {
                                    bettingType = 'Call';
                        ...
                        else
                        {
                            if(objectData.eState == 'FLOP' )
                            {
                                if(this.hasViableVision(objectData.handcard, objectData.tableCards.slice(0,3), objectData.eState)){
                                    ...
                                    {
                                        if (canRaise && randomType < 0.3) {
                                            bettingType = 'Full';
                                            iCallAmount = Math.floor(parseInt(objectData.iCallAmount)+(parseInt(objectData.iCallAmount) + parseInt(objectData.iTotalBettingCoin)));
                                        } else if (canRaise && randomType < 0.6) {
                                            bettingType = 'Half';
                                            iCallAmount = Math.floor(parseInt(objectData.iCallAmount)+(parseInt(objectData.iCallAmount) + parseInt(objectData.iTotalBettingCoin / 2)));
                                        } else {
                                            this.fRandomTime = 1;
                        ...
                        console.log(`iRobot~!!!!! iCallAmount : ${iCallAmount}`);
                        if (bettingType == 'Raise' || bettingType == 'Call' || bettingType == 'Half' || bettingType == 'Full') {
                    ...
                    else
                    {
                        ...
                        if(objectData.iCallAmount > 0)
                        {
                            ...
                            {
                                // this.fRandomTime =1 + Math.random()*2;
                                this.fRandomTime =1 + Math.random()*1;
                                console.log(`##### PREFLOP`);
                                if (this.isHoleCardInRange(holeCards, 'CO')) {
                                    ...
                                    if(this.isHoleCardInRange(holeCards, 'PREFLOP'))
                                    {
                                        if (objectData.iCallAmount == 0) {
                                            ...
                                            else if ( bettingType == 'Half' ) {
                                                iCallAmount = Math.floor(parseInt(objectData.iCallAmount)+(parseInt(objectData.iCallAmount) + parseInt(objectData.iTotalBettingCoin / 2)));
                                            }
                                            else if( bettingType == 'Full' )
                                            {
                                                iCallAmount = Math.floor(parseInt(objectData.iCallAmount)+(parseInt(objectData.iCallAmount) + parseInt(objectData.iTotalBettingCoin)));
                                            } 
                                            else {
                                        ...
                                        else if (objectData.iCallAmount > 0 && objectData.iCallAmount <= parseInt(objectData.iDefaultCoin*50)) {
                            ...
                            else if(objectData.eState == 'FLOP' )
                            {
                                console.log(`##### FLOP`);
                                if(this.hasViableVision(objectData.handcard, objectData.tableCards.slice(0,3), objectData.eState)){
                                    if(iCallAmount <= parseInt(objectData.iDefaultCoin*40))
                                    {
                                        bettingType = 'Call';
                            ...
                            else if(objectData.eState == 'TURN')
                            {
                                console.log(`##### TURN`);
                                if(this.hasViableVision(objectData.handcard, objectData.tableCards.slice(0,4), objectData.eState)){
                                    if(iCallAmount <= parseInt(objectData.iDefaultCoin*50))
                                    {
                                        bettingType = 'Call';
                                    }
                                    else if(iCallAmount == 0)
                ...
                this.iCallAmount = 0;
                this.strBetting = 'Fold';
                console.log("BOT BETTING ERROR:"+error);
            }
        });
        ...
        this.socket.on('SM_Result', (listResult, listWinCards, strWinnerHand, strWinnerDescr, cPlayingUser, listPots) => {
            ...
            {
                if(listResult[i].strID == this.account.strID)
                {
                    let iCash = parseInt(listResult[i].iCash)+parseInt(listResult[i].iCoin);
                    this.iCoin = listResult[i].iCoin;
                    this.account.iCash = iCash;
                }
            }
        ...
        this.socket.on('SM_RebuyIn', (listObject) => {
            ...
            {
                if(listObject[i].eUserType == 'JOKER' && listObject[i].bReserveExit == true)
                {
                    // this.socket.emit('CM_LeaveGame',listObject[i].strID)
                    console.log(listObject[i]);
                    window.removeRobot(listObject[i]);
                }
            }
        });
        
        this.socket.on('SM_Remove', (strID) => {
            console.log(`SM_Remove`);
            console.log(strID);
            let robotObject = {strID:strID};
            window.removeRobot(robotObject);
        });
