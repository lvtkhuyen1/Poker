...
//자동패스 가능한지 확인하는 함수
const shouldAutoPass = (listHandCard, listTableCard) => {
...
const isValidNextPlay = (listSelectCard, listHandCard) => {
...
// 서로 족보 비교
const canPlay = (listHandCard, listTableCard) => {
     if (listHandCard.length !== listTableCard.length) {
        return false;
    }
...
const canNextPlay = (listSelectCard, listTableCard, listHandCard) => {
...
const generateValidCombinations = (handCards, tableCards, bHigherCard) => {
...
const isLowestCardValidPlay = (listHandCard, listSelectCard) => {
    // 가장 낮은 카드를 찾는 함수
  ...
  const findLowestCard = (listHandCard) => {
  ...
  // 가장 높은 카드를 찾는 함수
const findHighestCard = (listHandCard) => {
    if (listHandCard.length === 0) {
        return []; // 카드가 없을 경우 [] 반환
      }
